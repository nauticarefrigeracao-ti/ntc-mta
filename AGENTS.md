...
## Como reconstruir o contexto deste projeto (leia isto primeiro)

Este projeto guarda a memória em três camadas. Leia nesta ordem:

1. **Este arquivo** — as regras que não se negociam. É a única fonte;
   `CLAUDE.md` e `GEMINI.md` só apontam para cá.

2. **`git log --oneline`** — o índice. Cada assunto de commit descreve o
   DEFEITO ou a DECISÃO, não o arquivo tocado. Use para escolher o que abrir.

3. **`git show <hash>`** nos commits que o índice apontou. O corpo traz
   sintoma → medição (endpoint, amostra, data) → decisão → alternativa
   descartada → invariante criada.

**NÃO** leia o histórico inteiro com corpo completo — medido neste repositório
em 07/08/2026: `--oneline` inteiro dá ~20k tokens e cabe; com corpo completo
dá ~61k, que é 30% de uma janela de 200k gasta antes da primeira pergunta.
Índice primeiro, corpo sob demanda.

Consultas dirigidas, que reconstroem um mapa inteiro em poucos tokens:

```bash
git log --pretty='%h %s | %(trailers:key=Invariante,valueonly)'   # defeito -> teste
git log --grep='Medido-em' --oneline                              # o que foi medido
git log -S'<símbolo>' --oneline                                   # quando nasceu e por quê
```

O processo está em `harness/PADRAO.md`. Leia **sob demanda**, antes de mudar
qualquer coisa — não carregue no início da sessão.
