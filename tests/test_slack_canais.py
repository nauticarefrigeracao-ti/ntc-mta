...
def test_canal_existente_e_reaproveitado():
    ...
    with patch.object(slack_client, "listar_canais",
                      return_value=[{"name": "sac", "id": "C111"},
                                    {"name": "sac-fechamento", "id": "C222"}]):
...
def test_canal_inexistente_e_criado_e_entra():
    with patch.object(slack_client, "listar_canais", return_value=[{"name": "sac", "id": "C111"}]):
...
def test_prefixo_cerquilha_e_opcional():
    with patch.object(slack_client, "listar_canais", return_value=[{"name": "sac-fechamento", "id": "C222"}]):
